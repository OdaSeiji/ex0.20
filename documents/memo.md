...
押出種別の表示も出来るようになった。次は、ダイスマーク類の問題がある型の表示。これはややこしい。最終押出日より後に、`dies_status = 31`になった金型という事。つまり、最終押出後に`dies_status = 31`になった`dies_id`の表を作るという事。最終押出日は既にあるので、型別で、最終情報が`dies_status=31`になったやつを探す。
